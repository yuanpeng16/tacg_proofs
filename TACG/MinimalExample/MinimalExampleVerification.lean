...
node h takes exactly two distinct values on all samples), then training
correctness implies test correctness (compositional generalization).
...
lemma h_a_neq_h_b (M : ExampleModel) (h_correct : ∀ s ∈ train_set, M.out s = y s) :
    M.h a ≠ M.h b := by
  intro h_eq
...
/-- Similarly, h(c) ≠ h(d). -/
private lemma h_c_neq_h_d (M : ExampleModel) (h_correct : ∀ s ∈ train_set, M.out s = y s) :
    M.h c ≠ M.h d := by
  intro h_eq
...
private lemma h_range (M : ExampleModel) (h_min : M.IsMinimized)
  ...
  have h_ab_ne : M.h a ≠ M.h b := h_a_neq_h_b M h_correct
...
private lemma h_c_eq_b (M : ExampleModel) (h_min : M.IsMinimized)
  ...
  have h_c_ne_a : M.h c ≠ M.h a := by
    ...
    apply h_ce
    rw [← h_ae, h_eq]
  cases h_range M h_min h_correct c with
...
private lemma h_d_eq_a (M : ExampleModel) (h_min : M.IsMinimized)
    (h_correct : ∀ s ∈ train_set, M.out s = y s) :
    M.h d = M.h a := by
  have h_c_eq_b := h_c_eq_b M h_min h_correct
  have h_db : M.h d ≠ M.h b := by
    intro h_eq
    have h_eq_dc : M.h d = M.h c := by
      rw [h_eq, h_c_eq_b]
    exact h_c_neq_h_d M h_correct h_eq_dc.symm
  cases h_range M h_min h_correct d with
  | inl hd => exact hd
...
lemma h_z_correspondence_train (M : ExampleModel) (h_min : M.IsMinimized)
    (h_correct : ∀ s ∈ train_set, M.out s = y s) (s : Sample) (hs : s ∈ train_set) :
    (M.h s = M.h a ∧ z s = 0) ∨ (M.h s = M.h b ∧ z s = 1) := by
  have h_ae : M.h a = M.h e := M.h_depends a e (by simp [x₁, x₂])
  have h_bf : M.h b = M.h f := M.h_depends b f (by simp [x₁, x₂])
  have h_c_eq_b := h_c_eq_b M h_min h_correct
  have h_d_eq_a := h_d_eq_a M h_min h_correct
  cases s with
...
private lemma h_inj_wrt_z_train (M : ExampleModel) (h_min : M.IsMinimized)
  ...
  have h_eq_a_iff_z_0 : ∀ s ∈ train_set, (M.h s = M.h a ↔ z s = 0) := by
    ...
    constructor
    · intro h
      have hz := h_z_correspondence_train M h_min h_correct s hs
      cases hz with
      | inl hz0 => exact hz0.2
      | inr hz1 => exact False.elim (h_a_neq_h_b M h_correct (h.symm.trans hz1.1))
    · intro hz0
      have hz := h_z_correspondence_train M h_min h_correct s hs
      cases hz with
      | inl hz1 => exact hz1.1
  ...
  have h_eq_b_iff_z_1 : ∀ s ∈ train_set, (M.h s = M.h b ↔ z s = 1) := by
    ...
    · intro h
      have hz := h_z_correspondence_train M h_min h_correct s hs
      cases hz with
      | inl hz0 => exact False.elim (h_a_neq_h_b M h_correct (hz0.1.symm.trans h))
      | inr hz1 => exact hz1.2
    · intro hz1
      have hz := h_z_correspondence_train M h_min h_correct s hs
      cases hz with
  ...
  · have hz1 := h_z_correspondence_train M h_min h_correct s₁ hs₁
    cases hz1 with
...
private lemma h_g_eq_b (M : ExampleModel) (h_min : M.IsMinimized)
  ...
  rw [h_g_eq_c]
  exact h_inj_wrt_z_train M h_min h_correct c (by decide) b (by decide) (by simp [z, x₁, x₂])
...
private lemma h_h_eq_a (M : ExampleModel) (h_min : M.IsMinimized)
  ...
  rw [h_h_eq_d]
  exact h_inj_wrt_z_train M h_min h_correct d (by decide) a (by decide) (by simp [z, x₁, x₂])
...
private lemma test_g_correct (M : ExampleModel) (h_min : M.IsMinimized)
    (h_correct : ∀ s ∈ train_set, M.out s = y s) :
    M.out g = y g := by
  have h_eq : M.h g = M.h b := h_g_eq_b M h_min h_correct
...
private lemma test_h_correct (M : ExampleModel) (h_min : M.IsMinimized)
    (h_correct : ∀ s ∈ train_set, M.out s = y s) :
    M.out h = y h := by
  have h_eq : M.h h = M.h a := h_h_eq_a M h_min h_correct
...
In the minimal example task, minimized representation (minimum entropy)
suffices for compositional generalization: if the model is correct on training
samples, then it is correct on test samples.
-/
theorem minimal_example_verification (M : ExampleModel) (h_min : M.IsMinimized) :
  ...
  cases hs
  · subst s
    exact test_g_correct M h_min h_correct
  · subst s
    exact test_h_correct M h_min h_correct
